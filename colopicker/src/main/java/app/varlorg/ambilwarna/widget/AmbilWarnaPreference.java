package app.varlorg.ambilwarna.widget;
...
import app.varlorg.ambilwarna.AmbilWarnaDialog;
import app.varlorg.ambilwarna.R;
...
public class AmbilWarnaPreference extends Preference {
	...
	public AmbilWarnaPreference(Context context, AttributeSet attrs) {
		...
		resetValue = ta.getInteger(R.styleable.AmbilWarnaPreference_resetValue, value);
		Log.d("app.varlorg.unote.debug", " AmbilWarnaPreference " + this.value);
	...
	@Override protected void onClick() {
		new AmbilWarnaDialog(getContext(), value, supportsAlpha, new AmbilWarnaDialog.OnAmbilWarnaListener() {
			...
			@Override
			public void onReset(AmbilWarnaDialog dialog, int color) {
				if (!callChangeListener(color)) return; // They don't want the value to be set
				if (resetValue != 0)
					value = resetValue;
				persistInt(value);
